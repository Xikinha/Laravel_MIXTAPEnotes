            ...
            <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                <div class="grid gap-6 sm:grid-cols-2">
                    <div class="relative z-0 col-span-2">
                        <input type="text" name="email" class="peer block w-full appearance-none border-0 border-b border-gray-500 bg-transparent py-2.5 px-0 text-sm text-gray-900 focus:border-green-600 focus:outline-none focus:ring-0" placeholder=" " required/>
                ...
                <div class="flex flex-col items-center justify-center py-4">
                    <button type="submit" class="mt-5 rounded-md bg-black px-4 py-2 text-white hover:bg-green-600 hover:text-black focus:outline-none focus:text-white focus:bg-gray-700">Login</button>
                    <p class="mt-4 content-center text-xs"><strong>Not registered?<span><a class="content-center text-xs text-green-600 hover:text-gray-900 focus:outline-none" href="{{ route('register') }}"> Create your account here.</a></span></strong></p>
                </div>
            </form>
        </div>
    </div>
